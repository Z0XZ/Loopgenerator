...
class lageOppgaver:
    ...
    def andregradslikninger(self):
        ...
        for i in range(self.antall):
            self.a = random.randint(
                int(round(self.startverdi / 2)), int(round(self.sluttverdi / 2))
            )

            self.b = random.randint(
                int(round(self.startverdi / 2)), int(round(self.sluttverdi / 2))
            )

            self.b_written = str(abs(self.b)) + "x"
            if self.b == 0:
                self.sign_b = ""
                self.b_written = ""
            elif self.b < 0:
            ...
            self.c = random.randint(
                int(round(self.startverdi / 2)), int(round(self.sluttverdi / 2))
            )

            self.c_written = abs(self.c)
            if self.c == 0:
                self.sign_c = ""
                self.c_written = ""
            elif self.c < 0:
            ...
            self.x2 = (
                self.b * (-1) - math.sqrt(self.b**2 - 4 * self.a * self.nyC)
            ) / (2 * self.a)
            ...
            if random.randint(1, 3) == 1:
                self.likning = f"{self.sign_a}{self.a_written}x²{self.sign_b}{self.b_written}{self.sign_nyC}{self.nyC_written}=0"
            else:
                self.likning = f"{self.sign_a}{self.a_written}x²{self.sign_b}{self.b_written}{self.sign_c}{self.c_written}={self.d}"
            ...
            if self.regnestykke == 3:
                ...
                if self.b >= 0:
                    ...
                    if self.c >= 0:
                        if self.d > 0:
                            self.uttrykk = "({} - {}) · {} + {}".format(
                                self.a, self.b * (-1), self.c, self.d
                            )
                        elif self.d == 0:
                            self.uttrykk = "({} - {}) · {}".format(
                                self.a, self.b * (-1), self.c
                            )
                        else:
                            self.uttrykk = "({} - {}) · {} - {}".format(
            ...
            if self.regnestykke == 5:
                ...
                if self.b >= 0:
                    ...
                    if self.c >= 0:
                        if self.d > 0:
                            self.uttrykk = "{} + ({} - {} · {})".format(
                                self.a, self.d, self.b * (-1), self.c
                            )
                        else:
                            self.uttrykk = "{} - ({} + {} · {})".format(
                                self.a, self.d * (-1), self.b * (-1), self.c
                            )
                    else:
                        if self.d > 0:
                            self.uttrykk = "{} + ({} - {} · ({}))".format(
...
def tegnIndreSirkelMedPil(radiusStor, radiusLiten, antallPiler):
    ...
    pdf.ellipse(
        senterBredde - radiusPilSirkel,
        senterHøyde - radiusPilSirkel,
        2 * radiusPilSirkel,
        2 * radiusPilSirkel,
        "D",
    )
    ...
    if antallPiler >= 4:
        ...
        while sluttpunktPilH_x - startpunktPilH_x > 0.1:
            pdf.line(
                startpunktPilH_x, startpunktPilH_y, sluttpunktPilH_x, startpunktPilH_y
            )
    ...
    if antallPiler >= 3:
        ...
        while sluttpunktPilV_x - startpunktPilV_x > 0.1:
            pdf.line(
                startpunktPilV_x, startpunktPilV_y, sluttpunktPilV_x, startpunktPilV_y
            )
    ...
    if antallPiler >= 2:
        ...
        while sluttpunktPilØ_y - startpunktPilØ_y > 0.1:
            pdf.line(
                startpunktPilØ_x, startpunktPilØ_y, startpunktPilØ_x, sluttpunktPilØ_y
            )
    ...
    if antallPiler >= 1:
        ...
        while sluttpunktPilN_y - startpunktPilN_y > 0.1:
            pdf.line(
                startpunktPilN_x, startpunktPilN_y, startpunktPilN_x, sluttpunktPilN_y
            )
...
def tegnSirklerMedTall(antallOppgaver, tall):
    pdf.add_page()
    ...
    if antallOppgaver > 9:
        radiusStor = (
            senterHøyde - vinkelGrad * 1.5
        )  # radius fra senter av arket og ut til sirklene
    else:
    ...
    pdf.set_font("Helvetica", "B", tekstStørrelse)
    ...
    pdf.cell(radiusStor * 2, radiusStor * 2, "Fasit", 0, align="C")
...
def tegnSirklerUtenTall(antallOppgaver):
    ...
    if antallOppgaver < 20 and antallOppgaver > 15:
        tegnIndreSirkelMedPil(radiusStor + radiusLiten / 2, radiusLiten, 4)
    elif antallOppgaver <= 15 and antallOppgaver > 7:
        tegnIndreSirkelMedPil(radiusStor + radiusLiten / 2, radiusLiten, 2)
    elif antallOppgaver >= 20:
        tegnIndreSirkelMedPil(radiusStor, radiusLiten, 4)
...
def oppgavegenerator(antallOppgaver):  # Alle sidene med oppgaver, nummereringer og svar
    ...
    pdf.add_font(
        "sysfont",
        "",
        r"C:\Users\joboe\AppData\Local\Microsoft\Windows\Fonts\DejaVuSerif.ttf",
    )
    ...
    for i in range(f):
        ...
        pdf.ellipse(
            10, høyde - dLO - 10, dLO, dLO, "D"
        )  # Om man ønsker sirkel rundt tallet kan denne brukes. Da må rammen fjernes i cellen med tekst.
        pdf.set_font_size(dLO)
        pdf.set_xy(10, høyde - dLO - 10)
        nummer = str(int(oppgavenummerIRekkefølge[i]))
        pdf.cell(dLO, dLO, nummer, 0, align="C")
        ...
        pdf.cell(bredde - 20, 4 * høyde / 5, oppgaveTekst, 0, align="C")
        pdf.set_xy(10, 0)
        pdf.cell(bredde - 20, høyde / 5, svarPåForrigeOppgave, "B", align="C")
...
knapp1 = tk.Button(master, text="Lineære likninger", command=oppgaveLin).grid(
    row=6, column=0, pady=4
)
knapp2 = tk.Button(master, text="Andregradslikninger", command=oppgaveAnd).grid(
    row=7, column=0, pady=4
)
knapp3 = tk.Button(master, text="Regnerekkefølge", command=oppgaveRegn).grid(
    row=8, column=0, pady=4
)
"""
